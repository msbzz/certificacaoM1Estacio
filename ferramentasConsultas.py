...
def consultarFerramentas():
    ...
    def getFilterTreeView(listaTodos):
        ...
        if len(lsReturnAll)==0:
            lsReturnAll=getFilter(listaTodos,descricao,listaFiltros['descricao'])
        else:
            lsReturnAll=getFilter(lsReturnAll,descricao,listaFiltros['descricao'])

        if len(lsReturnAll)==0:
            lsReturnAll=getFilter(listaTodos,fabricante,listaFiltros['fabricante'])
        else:
            lsReturnAll=getFilter(lsReturnAll,fabricante,listaFiltros['fabricante'])

        if len(lsReturnAll)==0:
            lsReturnAll=getFilter(listaTodos,material,listaFiltros['material'])
        else:
            lsReturnAll=getFilter(lsReturnAll,material,listaFiltros['material'])
    ...
    lsDados = cDados.OpenReadXLSX(nomePlanilhaDeConsulta, 'ferramentas', Cab)
