...
def cadastroFerramentas():
    ...
    labelLayout = Label(master,image=imgLayout)
    labelLayout.place(x=0, y=0)
    Label(master, text="CADASTRO DE FERRAMENTAS", font= ("Calibri",20)).grid(row=0,column=0,columnspan=40,padx='280',pady='20')
    #
    
    
    Label(master,text='').grid(row=1,column=0,rowspan=10)
    lin=2
    
    Label(master, text="CODIGO", font=("Calibri", 15)).grid(row=lin,column=0,sticky='w',padx='5' )
    eCodigo=Entry(master,bd=2,font=('Calibri',15))
    eCodigo.grid(row=lin,column=1)
    
    Label(master, text="DESCRICAO", font=("Calibri", 15)).grid(row=lin+1,column=0,sticky='w',padx='5')
    eDescricao=Entry(master,bd=2,font=('Calibri',15))
